...
def create_mock_config_for_content_tests(
    views=None,
    default_view="raw",
    app_title="Test",
    ai_provider="gemini",
    ...
    """Create a mock configuration object for content processing tests with sensible defaults."""
    mock_config = Mock()

    # AI configuration
    mock_config.ai = Mock()
    mock_config.ai.provider = ai_provider
    mock_config.ai.model = ai_model
    mock_config.ai.api_key_env = ai_api_key_env
    mock_config.ai.temperature = ai_temperature
    mock_config.ai.max_tokens = ai_max_tokens

    # Views configuration - use provided views or simple defaults
    if views is None:
        views = [
            Mock(id="raw", enabled=True, auto_load=True),
            Mock(id="summary", enabled=True, auto_load=False),
        ]
    mock_config.views = views

    # Settings configuration
    mock_config.settings.default_view = default_view
    mock_config.settings.app_title = app_title

    return mock_config


def create_mock_view(view_id, auto_load=True, enabled=True):
    """Create a mock view object with the given properties."""
    mock_view = Mock()
    mock_view.id = view_id
    mock_view.auto_load = auto_load
    mock_view.enabled = enabled
    return mock_view
...
class TestContentFetching:
    ...
    def setup_method(self):
        ...
        with patch("spegel.main.load_config") as mock_load_config:
            mock_config = create_mock_config_for_content_tests()
    ...
    async def test_fetch_and_display_url_history_management(self):
        ...
        urls = ["https://first.com", "https://second.com", "https://first.com"]  # Duplicate
...
class TestViewProcessing:
    ...
    def setup_method(self):
        ...
        with patch("spegel.main.load_config") as mock_load_config:
            ...
            mock_raw_view = create_mock_view("raw", auto_load=True, enabled=True)
            mock_summary_view = create_mock_view("summary", auto_load=True, enabled=True)
            mock_analysis_view = create_mock_view("analysis", auto_load=False, enabled=True)

            mock_config = create_mock_config_for_content_tests(
                views=[mock_raw_view, mock_summary_view, mock_analysis_view]
            )
            mock_load_config.return_value = mock_config
    ...
    async def test_process_all_views_parallel_auto_load_only(self):
        ...
        with patch.object(self.app, "_process_single_view", new_callable=AsyncMock) as mock_process:
    ...
    async def test_update_view_content_llm_view(self):
        ...
        # Mock streaming
        async def mock_stream_view(view, raw_html, llm_client, url):
            yield "This is "
            yield "a summary."
...
class TestTabManagement:
    ...
    def setup_method(self):
        """Set up test fixtures."""
        with patch("spegel.main.load_config") as mock_load_config:
            mock_config = create_mock_config_for_content_tests(views=[])
            mock_load_config.return_value = mock_config
    ...
    def test_reset_tab_names(self):
        ...
        # Mock TabbedContent and Tab objects
        mock_tab1 = Mock()
        mock_tab2 = Mock()
        ...
        # Should update labels for both tabs via Tab objects
